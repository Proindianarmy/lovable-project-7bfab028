...
function AuthPage() {
  ...
  // Show warning if redirected here from a protected route
  const redirected =
    typeof window !== "undefined" &&
    document.referrer === "" &&
    window.location.search.includes("blocked");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === "signup") {
      ...
      if (!match) {
        setErrors({
          general: "No account found with these credentials. Please create an account first.",
        });
        return;
      }
      localStorage.setItem("isLoggedIn", "true");
          ...
          <div className="rounded-2xl border border-border bg-card p-8 shadow-sm">
            ...
            {redirected && (
              <div className="mb-4 flex items-start gap-2 rounded-lg border border-yellow-300 bg-yellow-50 px-4 py-3 text-sm text-yellow-800">
                ...
                <span>You must be signed in to access that page. Please sign in or create an account.</span>
              </div>
            )}
